...
class StatusController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if(isset($_POST['Status']))
        {
            ...
            $user = User::model()->find(array('condition' => "email='".Yii::app()->user->email."'"));
            ...
            if($model->save()){
                if($imageUploadFile !== null) // validate to save file
                    $imageUploadFile->saveAs('uploads/' . $model->image);
    ...
    public function actionIndex()
    {
        ...
        if ($user_type == "admin"){
            ...
            $data = User::model()->findAll(array('condition' => "boss='".Yii::app()->user->id."'"));
            $my_clients = array();
            foreach ($data as $client)
                array_push($my_clients, $client->id);
            $str = implode(',',$my_clients);
            $dataProvider=new CActiveDataProvider('Status', array(
                'criteria' => array('condition' => "(privacy='public' && (user_type='admin' || user_id IN ($str))) || user_id='$user->id' || privacy='agents only' || share_with LIKE '%$user->id%'")));
        }else{
            $dataProvider=new CActiveDataProvider('Status', array(
                'criteria' => array('condition' => "(privacy='public' && (user_type='client' || user_id='$user->boss')) || user_id='$user->id' || privacy='clients only' || share_with LIKE '%$user->id%'")));
        }
    ...
    {
        $model=Status::model()->find(array('condition' =>"id='$id'"));
        if($model===null)
